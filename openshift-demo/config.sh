...
# if you want to retreive your plantext auth token, use: 
# SCRIPT_ENCRYPTION_KEY=${OPENSHIFT_RHSADEMO_USER_PASSWORD_DEFAULT} ; GITHUB_AUTHORIZATION_TOKEN_OPENSHIFT_DEMO_PLAINTEXT=`echo ${GITHUB_AUTHORIZATION_TOKEN_OPENSHIFT_DEMO_CIPHERTEXT} | openssl enc -d -a | openssl enc -d -aes-256-cbc -k ${SCRIPT_ENCRYPTION_KEY} `
